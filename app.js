...
//send a custom message by saying someone user name or saying a specific word (can be done multiple time, consult Examples.js)
  if (message.startsWith(`I`)) {
   bot.message.send(`Shush! I love you more 👁👄👁🔪 `);
  };


  if (message.startsWith(`Bot`)) {
   bot.message.send(`Hello, the person who created me is @Atekinz, follow him.`);
  };
...
bot.on('playerJoin', async (user) => {
  ...
  if (user.username === 'Atekinz') {
    bot.message.send("my owner joined the room! @Atekinz 🤖👑")
  }
...
bot.on('playerJoin', (user) => {
  ...
  bot.message.send(`@${user.username} Welcome to Club Bunny by iBunnyx! Have Fun, Dance, Invite your Friends and Tip The Jar!`);
});
...
bot.on('playerLeave', (user) => {
  ...
  bot.message.send(`See You Later @${user.username}!`)
});
